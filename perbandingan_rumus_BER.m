...
%% EbN0 berdasar SNR
% jadi semua pakai rumus/simulasi EbN0
% menggunakan parameter 802.11ah, MCS0
bitrate = 4e6;  % 1.3 Mbps adalah uncoded dari 0,65 Mbps
                % kalau 4 Mbps ini hasilnya sama dengan yang SNR.
                % jadi bitrate adalah 2x bandwidth kalau ingin sama.
bandwidth = 2e6; % 2 MHz
SNRdB=-10:8;
Eb_N0_dB = SNRdB - 10 * log10(bitrate/bandwidth);
...
figure
semilogy(SNRdB,theoryBerAWGN_pake_erfc,'b-o','LineWidth',1, 'MarkerIndices',1:2:length(Eb_N0_dB));
hold on
semilogy(SNRdB,theoryBerAWGN_pake_qfunc,'rp-','LineWidth',1, 'MarkerIndices',1:3:length(Eb_N0_dB));
semilogy(SNRdB,theoryBerRayleigh,'g-o','LineWidth',1, 'MarkerIndices',1:2:length(Eb_N0_dB));
semilogy(SNRdB,theoryBerRayleigh_pake_berfading,'kp-','LineWidth',1, 'MarkerIndices',1:3:length(Eb_N0_dB));
semilogy(SNRdB,rasio_error_awgn_snr_sendiri,'y<-','LineWidth',1, 'MarkerIndices',1:5:length(Eb_N0_dB));
semilogy(SNRdB,rasio_error_rayleigh_snr_sendiri,'c>-','LineWidth',1, 'MarkerIndices',1:5:length(Eb_N0_dB));
grid on
legend("theoryBerAWGN erfc", "theoryBerAWGN qfunc", ...
    "theoryBerRayleigh rumus", "theoryBerRayleigh berfading", ...
    "Sim AWGN", "Sim Rayleigh");
